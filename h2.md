  ...
  Ympäristö: Kali Linux Virtualboxsissa ja Windows 11 host OS:ssä tehty tehtävä 

  Päivämäärä: 01/11/2025 18:00-21:20, 02/11/2025 19:00-22:00, Päivitetty versio, sain palautetta että analyysit puuttuivat, korjattu 02/11/2025 22:30-xx:xx
  ...
  Pyramidin pohjalla olevat indikaattorit kuten IP-osoitteet ja hashit ovat hyökkääjän helppo vaihtaa, kun taas pyramidin huipulla olevat TTPs (taktiikka, tekniikat ja menettelytavat) vaativat hyökkääjältä merkittävää työtä ja uudelleenkoulutusta muuttaakseen.
  ...
  Malli auttaa analyytikoita ymmärtämään hyökkäysten välisiä yhteyksiä ja suhteiden kautta "pivotoimaan" eli siirtymään yhdestä elementistä toiseen löytääkseen uutta tietoa ja tunnistaakseen hyökkääjien toimintatapoja.
...
---Päivitys 02/11/2025 22:30-xx:xx---
  
  * 127.0.0.1 = IP-osoite
  * Pyynnön aika 31/Oct/2025:08:35:32 -0400
  * GET/favicon.ico HTTP/1.1 pyyntötyyppi (GET), käytännössä pyydetään selainta antamaan ikoni
  * Palvelin vastaa 404, eli ikonia ei ole (200 olisi ollut ok ja lähetetään)
  * 404 vastauksen koko tavuina, eli 404 viestin koko oli 487
  * http://localhost  = kertoo miltä sivulta pyyntö tuli
  * "Mozilla/5.0 (X11; Linux x86_64; rv:128.0) Gecko/20100101 Firefox/128.0" = User agentti, kertoo mikä selain teki pyynnön.
  ...
  Kuvassa näkyy nmap skannauksen tulokset. Tuloksessa erotellaan palvelimen käyttöjärjestelmä (OS CPE & OS details), traceroute ("Netwrok distance:" eli monta vaihtoa paketti on joutunut tekemään että se saapuu määränpäähän (minun tietokonelta -> palvelimelle), 

 *  Port : 80/tcp eli mitä porttia palvelin käyttää välittääkseen tietoa,
 *  State: "open", eli portti on auki,
 *  Service: http, palvelun tyyppi/nimi,
 *  version: Apache httpd pelvelimen versio.
...
<img width="1427" height="340" alt="image" src="https://github.com/user-attachments/assets/d3622bba-1f5f-4928-a026-739d736f67d9" />
  ...
  Tässä näkyy loopback feedback missä tietokone lähettää itselleen tiettyjä paketteja, sen voi tunnistaa siitä, että localhost lähettää localhostille (127.0.0.1) paketteja.

  Kuvassa on esim. nmaplowercheck mikä tarkoittaa Nmap:in luoma satunnainen HTTP-polku, jota käytetään testaamaan onko web-palvelin case-sensitive (erottaako se isot ja pienet kirjaimet).

  Jos palvelin palauttaa eri vastaukset: se on case sensitive, jos ei, se ei ole case sensitive. 

  Tämä auttaa nmappia arvaamaan mitä OS:sää kohde käyttää, esim. windowssi on case sensitive mutta linux ei ole. 

  Tällä voi myös hakee piiloutettuja hakemistoja.
...
<img width="858" height="289" alt="image" src="https://github.com/user-attachments/assets/d43d1ad6-4192-4c2d-be31-d4ba05d1fa4a" />
  ...
  Ensiksi muokkasin http.lua skriptiä, oletuksena on, koska apache pyörii http:llä niin minun pitäisi poistaa sieltä tunnistava tieto.

  Navigoin kyseiseen hakemistoon ja suoritin komennon 

  sudoedit /usr/share/nmap/nselib/http.lua

  Siitä etin nmappiin viittaavia stringejä kuten

 <img width="923" height="96" alt="image" src="https://github.com/user-attachments/assets/23475c90-9cc6-4ee7-85b3-6ebcffa4e776" />

  nmap scriptin enginen korvasin obama enginellä.

  Seuraava mitä pitää muokata on workstation name. Nimesin workstationiksi obama-gaming.

  <img width="839" height="151" alt="image" src="https://github.com/user-attachments/assets/760db5b9-6d5a-4030-88ad-7908f4543d4c" />

  En ole tässä viheessa varma, onko tämä tarpeellinen, kun nmapissa saa suoraa komentoriviltä määrittää mikä useragentti pitäisi olla, mutta en oikee keski muutakaan.

  Tallennuksen jälkeen yritin tehdä porttisannauksen uudestaan. Wiresharkkiin edelleen ilmestyy nmaplowercheck. 

  <img width="968" height="146" alt="image" src="https://github.com/user-attachments/assets/ce8ec7ed-51e1-4fb5-a981-4a734501e153" />
  
  Tässä vaiheessa päätin kysyä tekoälyltä neuvoa kun oma tieto ei riittänyt.

  Tekoäly ehdotti etsimään tietoa näistä kolmesta hakemistosta.

  <img width="630" height="161" alt="image" src="https://github.com/user-attachments/assets/4fbdf5bf-243b-4814-8ff5-4a30148901ef" />

  Tekoälyltä tuli myöskin toinen vinkki, miten paremmin etsitään tietoa hakemistosta. grep -rn "hakusana" tulostaa tiedoston missä stringi ilmestyy

  -rn = tarkistaa kaikki alihakemistot ja antaa oikean rivinumeron missä kyseinen stringi ilmestyi.

  <img width="851" height="65" alt="image" src="https://github.com/user-attachments/assets/28c747e3-0a11-4ae4-ad2d-56cf3f8b5808" />

  Löysin tiedostosta nmaplowercasein ja muokkasin sen obamalowercase ja obamauppercase

  <img width="714" height="102" alt="image" src="https://github.com/user-attachments/assets/3033a769-b7cb-4060-98eb-1bce75f453a2" />

  Tallensin ja lähdin suorittamaan nmappia uudestaan.
  
  nmaplowercasea ei enää näkynyt wiresharkissa porttiskannauksen jälkeen.

  <img width="967" height="542" alt="image" src="https://github.com/user-attachments/assets/29ecec90-286e-46b2-87ba-ddece9845702" />

  Tämän sijaan obama löytyi.

  <img width="966" height="268" alt="image" src="https://github.com/user-attachments/assets/e89f884e-205d-47d6-9bc2-1b58539a6b8e" />
  ...
  Tällä kertaa vapaehtoista tehtävää en tehnyt. 

  
  Lähteet:
  https://blog.smallsec.ca/nmap-scan-cheatsheet/
